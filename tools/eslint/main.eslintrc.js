...
// `extensionsGlob` is a glob pattern that matches `importExtensions`, not
...
module.exports = {
  ...
  // `babel-eslint` is required to support Flowtype (if used) and the
  ...
  overrides: [
    ...
    {
      files: [`src/**/browser/**/*.${extensionsGlob}`],
      env: {
        browser: true
      }
    },
    {
      files: [`src/**/node/**/*.${extensionsGlob}`],
      env: {
        node: true
      }
    }
